...
function initTyping(){
    ...
    let tyspeedchar = inputField.value.split("")[charIndex]
    if(charIndex < characters.length - 1 && timeLeft > 0){
        if(!isTyping){//once timer starts it won't start again on every inputclicked
            timer = setInterval(initTimer, 1000)
            isTyping = true;
        }
        if(tyspeedchar == null){//for backspace button
            charIndex--
            if(characters[charIndex].classList.contains('incorrect')){
                mistakes--;
            }
            characters[charIndex].classList.remove('correct','incorrect');//removing correct and incorrect classes
        }
        else{
            if(characters[charIndex].innerText === tyspeedchar){
        ...
        characters.forEach(span=> span.classList.remove('active')); //first we remove all active classes from all span and then add active class only actve class element
        characters[charIndex].classList.add('active')//for blinking line
        let wpm = Math.round((((charIndex - mistakes) / 5) / (maxTime - timeLeft)) * 60)//5 means asuming 5 chacracters = 1 word
        ...
        inputField.value = ""; //timer over per input value empty
        clearInterval(timer)
    }
}
